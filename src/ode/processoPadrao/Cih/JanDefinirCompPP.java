...
import ode.processoPadrao.Cih.JanDefinirProcessoPadrao.EventListnerDefinirCompPP;
...
public class JanDefinirCompPP {
	...
	class EventListnerSalvar implements EventListener {
		...
		public void onEvent(Event arg0) throws Exception {
			...
			if(comboBGranularidade.getText().compareTo("Processo Simples")==0){
				ObjTipo = comboBListaTipoDeProcessoSimples.getSelectedItem().getValue();
			}
	...
	private void preencherCombobox() {
		...
		for (KProcesso kprocesso : listaKProcesso) {
			comboItem = comboBListaTipoDeProcessoSimples.appendItem(kprocesso.getNome());
			comboItem.setValue(kprocesso);
		}
		
		for (KAtividade kAtividade : listaKAtividade) {
			comboItem =  comboBListaTipoDeMacroAtividade.appendItem(kAtividade.getNome());
