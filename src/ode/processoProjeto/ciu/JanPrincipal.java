...
import ode.processoPadrao.ciu.EnumAtividadeProcessoPadrao;
import ode.processoPadrao.ciu.JanEditarConhecimentosAtividadePP;
import ode.processoPadrao.ciu.JanPrincipal.EventListnerPripriedadesBasicas;
import ode.processoProjeto.ciu.CtrlDefinirProcessoProjeto;
import ode.processoProjeto.ciu.JanCore;
...
public class JanPrincipal extends JanCore {
	...
	// Construtor
	public JanPrincipal(CtrlDefinirProcessoProjeto ctrlDefinirProcessoProjeto) {
	...
	private void menu() {
		...
		Menu menu = new Menu("Processo de Projeto");
		...
		newItemBasicoMenuProjeto(popupDefCompPP, "A partir de um Processo Padrão existente", new EventListnerDefinirNovoCompPPr());
	//	newItemBasicoMenu(popupDefCompPP, "Atividade Padrão", new EventListenerDefinirAtividadePadrao());
		
		newItemBasicoMenu(popupEditar, "Processo de Projeto", new EventListenerEditarCompPProjeto());
	...
	class EventListenerEditarCompPProjeto implements EventListener {

		@Override
		public void onEvent(Event arg0) throws Exception {
			ctrl.abrirJanSelecionaProcessoProjeto();
		}
	}
	...
	public void conteudo() {
		this.setTitle("Definição de Processo de Projeto");
		criaArvoreCompPP();
	}
	...
	private void criaArvoreCompPP() {
		...
		if(ctrl.getcompPPrSelecionado() != null){
			geraTreeCompPPCompleta(ctrl.getcompPPrSelecionado().getPadraoBase()).setParent(tree.getTreechildren());
		}
	}
	...
	private Treeitem geraTreeCompPP(CompPPProcessoComplexo compPP) {
		...
		for (CompPPProcessoSimples compPPSimples : compPP.getProcessosSimples()) {
			...
			newItemBasicoMenu(menupopupCabecalho, "Excluir da composição do Processo", new EventExcluirDaComposicao(compPPSimples));
...
protected Treeitem geraTreeSubEPreAtividadeProcessoPadrao(AtividadeProcessoPadrao atividade, boolean DefinicaoConcluida) {
	...
	class EventExcluirDaComposicao implements EventListener {
		...
		public void onEvent(Event arg0) throws Exception {
			Messagebox.show("Deseja realmente excluir este da composição do componente: " 
			        + ctrl.getcompPPrSelecionado().getPadraoBase().getNome() + "?", "Finalização", Messagebox.OK
			        + Messagebox.CANCEL, Messagebox.QUESTION,
					new EventListener() {
						public void onEvent(Event evt) {
							if (((Integer) evt.getData()).intValue() == Messagebox.OK) {
									...
									if(compPP instanceof CompPPProcessoSimples){
										if(podeExcluirDaComposicao((CompPPProcessoSimples)compPP)){
											((CompPPProcessoComplexo)ctrl.getcompPPrSelecionado().getPadraoBase()).getProcessosSimples().remove(compPP);
											//ctrl.atualizarCompPP(ctrl.getcompPPrSelecionado().getPadraoBase());
										}else{
											try {
	...
	public boolean podeExcluirDaComposicao(CompPPProcessoSimples simples){
		...
		for(ElementoCompPP elemento: ctrl.getcompPPrSelecionado().getPadraoBase().getInterfaceCompPP().getEstruturaCompPP().getElementosCompPP()){
			if(elemento.isObrigatorio() ==  true){
				listaKAtividadeObrigatoria.add((KProcesso)elemento.getElementoConhecimento());
			}
		}
		
		if(ctrl.getcompPPrSelecionado().getPadraoBase().getCompPPBase() != null){
			for(ElementoCompPP elemento: ctrl.getcompPPrSelecionado().getPadraoBase().getCompPPBase().getInterfaceCompPP().getEstruturaCompPP().getElementosCompPP()){
				if(elemento.isObrigatorio() ==  true){
					listaKAtividadeObrigatoria.add((KProcesso)elemento.getElementoConhecimento());
				}
			}
...
/*		newItemBasicoMenu(menupopupContexto, "Selecionar Componente Base", new EventListener() {
		...
		if(ctrl.getcompPPrSelecionado().getPadraoBase() instanceof CompPPProcessoComplexo){
...
/*	protected void menusDeEdicaoMacroAtividade(MenupopupV menupopupContexto) {
	...
	private void newItemBasicoMenuProjeto(MenupopupV menupopup, String label, EventListener eventListner) {
		Menuitem menuItemDefinirCompPPr = new Menuitem();
		menuItemDefinirCompPPr.setParent(menupopup);
		menuItemDefinirCompPPr.setLabel(label);
		menuItemDefinirCompPPr.addEventListener("onClick", eventListner);
	}
	
	class EventListnerDefinirNovoCompPPr implements EventListener {

		@Override
		public void onEvent(Event arg0) throws Exception {
			ctrl.abrirJanDefinirCompPPr(true);
		}
	}
