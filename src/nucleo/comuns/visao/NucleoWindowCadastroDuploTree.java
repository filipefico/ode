...
/**
 * Classe abstrata que representa a base para uma janela de cadastro duplo com
 * uma árvore de objetos. Entende-se por cadastro duplo a possibilidade de
 * cadastrar dois tipos diferentes de objeto na mesma árvore, um objeto
 * organizador (análogo a Diretórios) e um objeto fim (análogo a Arquivos).
 * 
 * @author Alexandre G. N. Coelho
 * @param <D>
 *            Classe dos objetos organizadores (Diretórios).
 * @param <A>
 *            Classe dos objetos fim (Arquivos).
 */
public abstract class NucleoWindowCadastroDuploTree<D extends NucleoObjetoPersistenteImpl<Long, Long>, A extends NucleoObjetoPersistenteImpl<Long, Long>>
		extends NucleoWindow {

	/**
	 * Interface de aplicação usada para cadastros básicos dos objetos
	 * organizadores (Diretórios)
	 */
	protected NucleoAplCadastroBase<D> nucleoAplCadastroBaseOrganizador;

	/**
	 * Interface de aplicação usada para cadastros básicos dos objetos fim
	 * (Arquivos)
	 */
	protected NucleoAplCadastroBase<A> nucleoAplCadastroBaseObjetoFim;
	 ...
	 */
	public NucleoWindowCadastroDuploTree() {
		super();
	}
	 ...
	 */
	public NucleoWindowCadastroDuploTree(String[] colunasTree) {
		super();
	...
	protected void acaoMenuitemNovoOrganizador() {
		...
		NucleoWindowCadastroDados<D> win = (NucleoWindowCadastroDados<D>) Executions
		...
		try {
			...
			System.out.println("Erro ao abir a janela.");
		}
	}

	/**
	 * Cria um novo objeto organizador (Diretório) vazio.
	 * 
	 * @return Novo objeto organizador criado.
	 */
	protected abstract D criarNovoObjetoDadosOrganizador();
	...
	protected void acaoMenuitemNovoObjetoFim() {
		...
		NucleoWindowCadastroDados<A> win = (NucleoWindowCadastroDados<A>) Executions
				.createComponents(this
	...
	/**
	 * Cria um novo objeto fim (Arquivo) vazio.
	 * 
	 * @return Novo objeto fim criado
	 */
	protected abstract A criarNovoObjetoDadosObjetoFim();
		...
		while (itItensSelecionados.hasNext()) {
			...
			try {
				if (objeto.getClass() == this.criarNovoObjetoDadosOrganizador()
						.getClass()) {
					nucleoAplCadastroBaseOrganizador.excluir((D) objeto);
				} else {
					nucleoAplCadastroBaseObjetoFim.excluir((A) objeto);
				}
			} catch (Exception e) {
				exibirJanelaErro(e);
			}
	...
	public void setNucleoAplCadastroBaseOrganizador(
			NucleoAplCadastroBase<D> nucleoAplCadastroBase) {
		this.nucleoAplCadastroBaseOrganizador = nucleoAplCadastroBase;
	}

	public NucleoAplCadastroBase<D> getNucleoAplCadastroBaseOrganizador() {
		return this.nucleoAplCadastroBaseOrganizador;
	}

	public void setNucleoAplCadastroBaseObjetoFim(
			NucleoAplCadastroBase<A> nucleoAplCadastroBase) {
		this.nucleoAplCadastroBaseObjetoFim = nucleoAplCadastroBase;
	}

	public NucleoAplCadastroBase<A> getNucleoAplCadastroBaseObjetoFim() {
